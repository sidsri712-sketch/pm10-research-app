...
def fetch_weather():
try:
url = "[https://api.open-meteo.com/v1/forecast?latitude=26.85&longitude=80.94&hourly=temperature_2m,relative_humidity_2m,wind_speed_10m&forecast_days=1&timezone=Asia%2FKolkata](https://api.open-meteo.com/v1/forecast?latitude=26.85&longitude=80.94&hourly=temperature_2m,relative_humidity_2m,wind_speed_10m&forecast_days=1&timezone=Asia%2FKolkata)"
...
url = "[https://api.tomtom.com/traffic/services/4/flowSegmentData/absolute/10/json?point=](https://api.tomtom.com/traffic/services/4/flowSegmentData/absolute/10/json?point=)" + str(lat) + "," + str(lon) + "&key=" + TOMTOM_TOKEN
r = requests.get(url, timeout=10)
data = r.json()
...
st.metric("Emission Factor Used", "%.3f" % selected_factor)
