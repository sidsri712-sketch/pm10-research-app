...
# --------------------------------------------------
# DATA PIPELINE (SUPABASE VERSION)
# --------------------------------------------------
...
if run_hybrid or run_diag or predict_custom:
    ...
    if run_diag:
        ...
        res_diag, mae = run_diagnostics(df_live)
        c1, c2 = st.columns(2)
        with c1:
            fig, ax = plt.subplots()
            sns.regplot(data=res_diag, x="Actual", y="Predicted", ax=ax)
            st.pyplot(fig)
    ...
    if not df_history.empty:
        ...
        st.download_button("📥 Download Full Historical CSV", data=df_history.to_csv(index=False).encode("utf-8"), file_name="lucknow_pm10_history.csv", mime="text/csv")
